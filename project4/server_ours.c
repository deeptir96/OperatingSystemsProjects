...
int server_lookup(int pinum, char *name) {
  ...
  printf("server_lookup: no such file/dir\n");
...
int server_write(int inum, char* buffer, int block){
  ...
  if (writeToLog) {
    writeDataBlock(offset, wr_buffer);
    inode_new.size = (block+1)*4096; //TODO check this
  ...
  printf("Writing to imap piece %d\n", imapIdx);
...
int server_init(char *image) {
  ...
  if(access(image, F_OK) == 0) {
    ...
    printf("creating file\n");
      ...
      /* GW: how??? */
      int offset = cr->endLog;
      writeDirDataBlock(offset, dataBlock);
